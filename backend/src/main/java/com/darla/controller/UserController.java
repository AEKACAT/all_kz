...
//				OAuth2User userOauth = token.getPrincipal();
				...
				if (email == null) {
					return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Электрондық пошта аутентификация провайдерінде табылмады");
				}
    ...
    */
    ...
    public ResponseEntity<Response> createOrder(
    	...
    	if (orderRequest.getQuantity() <= 0) {
    		 throw new IllegalArgumentException("Саны 0-ден үлкен болуы керек");
