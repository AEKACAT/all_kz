...
public class ReviewService {
    ...
    public void addReview(ReviewDto reviewDto) {
        ...
        Product product = productRepository.findById(reviewDto.getProductId()).orElseThrow(
                () -> new NotFoundException("Өнім идентификаторымен табылмады:" + reviewDto.getProductId()));

        // Check if the user exists
        User user = userRepository.findById(reviewDto.getUserId())
                .orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + reviewDto.getUserId()));
    ...
    public List<ReviewDto> getReviewsByProduct(Long productId) {
        ...
        productRepository.findById(productId).orElseThrow(
                () -> new NotFoundException("Өнім идентификаторымен табылмады:" + productId));
    ...
    public ReviewDto getReviewById(Long id) {
        Review review = reviewRepository.findById(id).orElseThrow(() -> new NotFoundException("Идентификатормен шолу табылмады: " + id));
        ReviewDto reviewDto = new ReviewDto();
        reviewDto.setId(review.getId());
    ...
    public String deleteReview(Long id, Long userId) {
        // Check if the user exists
        userRepository.findById(userId).orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады:: " + userId));
        ...
        Product product = productRepository.findById(review.getProduct().getId())
                .orElseThrow(() -> new NotFoundException("Өнім идентификаторымен табылмады:" + review.getProduct().getId()));
    ...
    public String updateReview(Long reviewId, Double rating, String comment) {
        ...
        Review review = reviewRepository.findById(reviewId)
                .orElseThrow(() -> new NotFoundException("Идентификатормен шолу табылмады: " + reviewId));
