...
//				System.out.println("User logged in: " + user.getZipCode());
	...
	public UserDto getUserInfo(Long userId) {
		User user = userRepository.findById(userId)
				.orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + userId));
	...
	public String updateProfile(Long id, UserDto userDto) {
		User user = userRepository.findById(id)
				.orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + id));
	...
	public Response placeOrderWithRazorPay(OrderRequest orderRequest) {
		...
		User user = userRepository.findById(orderRequest.getUserId())
				.orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + orderRequest.getUserId()));
...
//				.orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + userId));
//		Product product = productRepository.findById(productId)
//				.orElseThrow(() -> new NotFoundException("Идентификаторы бар пайдаланушы табылмады: " + productId));
//		if (product.getStock() <= 0)
//			throw new RuntimeException("Product is out of stock");
//
